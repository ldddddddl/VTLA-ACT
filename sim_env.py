...
def make_sim_env(task_name):
    ...
    elif "maniskill" in task_name:
        from maniskill_env import make_maniskill_env

        # Extract env_id if possible, or default to PickCube-v1
        # For this specific task, we use PickCube-v1
        env_id = "PickCube-v1"
        # We can also map task_name to env_id if needed
        if "pickcube" in task_name:
            env_id = "PickCube-v1"

        env = make_maniskill_env(env_id=env_id)
    elif task_name == "verify_seed1":
        from maniskill_env import make_maniskill_env

        env = make_maniskill_env(env_id="PickCube-v1")
    else:
