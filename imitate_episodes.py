...
def main(args):
    ...
    is_sim = (
        task_name[:4] == "sim_"
        or task_name[:10] == "maniskill_"
        or task_name == "verify_seed1"
    )
    if is_sim:
        ...
        raise NotImplementedError("Real robot tasks not supported in this env")
    dataset_dir = task_config["dataset_dir"]
    num_episodes = task_config["num_episodes"]
    episode_len = task_config["episode_len"]
    camera_names = task_config["camera_names"]

    # Get state_dim from task config or use default
    state_dim = task_config.get("state_dim", 14)
    action_dim = task_config.get("action_dim", state_dim)
    lr_backbone = 1e-5
    backbone = "resnet18"

    # Set default values for ACT hyperparameters
    chunk_size = args.get("chunk_size") or 100
    kl_weight = args.get("kl_weight") or 10
    hidden_dim = args.get("hidden_dim") or 512
    dim_feedforward = args.get("dim_feedforward") or 3200
    if policy_class == "ACT":
        enc_layers = 4
        dec_layers = 7
        nheads = 8
...
def eval_bc(config, ckpt_name, save_episode=True):
    ...
    for rollout_id in range(num_rollouts):
        ...
        # Restore saved initial state if available (for overfitting tests)
        if "maniskill" in task_name.lower():
            from constants import SIM_TASK_CONFIGS

            task_config = SIM_TASK_CONFIGS.get(task_name, {})
            dataset_dir = task_config.get("dataset_dir", "")
            init_state_path = os.path.join(
                dataset_dir, f"episode_{rollout_id}_init_state.pkl"
            )
            if os.path.exists(init_state_path):
                ...
                # Get fresh observation after state restore
                obs = env.unwrapped.get_obs()
                agent_obs = obs.get("agent", {})
                qpos = agent_obs.get("qpos", ts.observation.get("qpos"))
                if hasattr(qpos, "cpu"):
                    qpos = qpos.cpu().numpy()
                if len(qpos.shape) > 1:
                    qpos = qpos[0]
                ts.observation["qpos"] = qpos
                print(f"Restored initial state from: {init_state_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--seed", action="store", default=42, type=int, help="seed", required=True
    )
    parser.add_argument(
        "--num_epochs", action="store", type=int, help="num_epochs", required=True
    )
    parser.add_argument(
        "--lr", action="store", default=1e-4, type=float, help="lr", required=True
    )
