...
class DETRVAEWithTactile(nn.Module):
    """
    DETR-VAE model with tactile modality support.

    This extends the original DETRVAE to incorporate tactile observations
    alongside visual and proprioceptive inputs.
    """
    ...
    def __init__(
        ...
        """
        Initialize the model with tactile support.

        Args:
            backbones: Visual backbone modules
            transformer: Transformer decoder
            encoder: Transformer encoder for VAE
            state_dim: Robot state/action dimension
            num_queries: Number of action queries (chunk size)
            camera_names: List of camera names
            tactile_dim: Dimension of tactile input
        ...
        if use_tactile:
            ...
            # Additional position embedding: proprio, latent, tactile
    ...
    def forward(self, qpos, image, env_state, actions=None, is_pad=None, tactile=None):
        ...
        Args:
            qpos: batch, qpos_dim
            image: batch, num_cam, channel, height, width
            env_state: None (not used currently)
            actions: batch, seq, action_dim (training only)
            is_pad: batch, seq (training only)
            tactile: batch, tactile_dim (optional tactile input)
        """
        ...
        if self.backbones is not None:
            ...
            if (
                ...
                and self.tactile_encoder is not None
            ):
                tactile_input = self.tactile_encoder(tactile)
            else:
                tactile_input = None
            ...
            env_state_embed = self.input_proj_env_state(env_state)
...
def build_with_tactile(args):
    """Build DETRVAE model with tactile support."""
    state_dim = getattr(args, "state_dim", 14)
    tactile_dim = getattr(args, "tactile_dim", 6)
    use_tactile = getattr(args, "use_tactile", True)

    # Build visual backbone
    backbones = []
    backbone = build_backbone(args)
    backbones.append(backbone)
